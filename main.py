...
def envemu(varname):
    with open(f"keys.env","r") as file:
        for line in file:
            if line.startswith(varname):
                var = line.split("=")[1].strip()
                print(f"Loaded {var} from keys.env")
                return var

#Инициализация бота
bottoken = envemu("BOT_TOKEN")
bot = telebot.TeleBot(bottoken) 

adminid1 = envemu("ADMIN_ID_1")
...
sock.connect((envemu('RCON_HOST'), int(envemu('RCON_PORT'))))

#SFTP
transport = paramiko.Transport((envemu('SFTP_HOST'), int(envemu('SFTP_PORT'))))
transport.connect(username=envemu('SFTP_USER'), password=envemu('SFTP_PASS'))

#MySQL
conn = mysql.connector.connect(
    host= envemu('MYSQL_HOST'),
    user= envemu('MYSQL_USER'),
    password= envemu('MYSQL_PASS'),
    database= envemu('MYSQL_DB')
)
...
try:
    ...
    result = mcrcon.login(sock, envemu('RCON_PASS')) #RCON логин 
    if result:
